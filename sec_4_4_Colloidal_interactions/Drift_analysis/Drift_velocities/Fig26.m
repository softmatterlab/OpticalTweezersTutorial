...
load([...filepath sottodir filesep 
    'data/histograms.mat'],'d_hist','d_hist0','d_hist1','d_hist_tot',...
...
%     ah_dri_shade{i,1} = axes('Position',[xs(1) 0 xwi 0]/totXpix1 + [0 ys(1)+(numrow-i)*(ys(2)+ywi) 0 ywi]/totYpix1,'ydir','reverse');
    ...
    ima = imread([...dirtif 
        'data/dri_para_' num2str(i) '_tif_new.tif']);
    image(ima)
...
%     ah_dri_shade{i,2} = axes('Position',[xs(1)+xs(2)+xwi 0 xwi 0]/totXpix1 + [0 ys(1)+(numrow-i)*(ys(2)+ywi) 0 ywi]/totYpix1,'ydir','reverse');
    ...
    ima = imread([ 'data/dri_perp_' num2str(i) '_tif_new.tif']);
    image(ima)
    axis off; box off; grid off;
    %axis on; box on;
...
for n = [1  3 4 6] %numel(k_list)
    ...
    load([...filepath sottodir filesep 
        'data/drift_diffu_mean_err_xi' num2str(T_exp(1,ixi)*10) '.mat'],...
...
%     options_drift_perp_expe={0.008,'o','Color','k','MarkerFaceColor',colo_light{mod(n-1,num_col)+1},'MarkerSize',3,'Linewidth',3};
    ...
    errorbar(B_expe{ik}(ifull)',dripar_av_expe{ik}(ifull)',dripar_std_expe{ik}(ifull)', 'o','Color','k','MarkerFaceColor',colo_light{mod(n-1,num_col)+1}, 'MarkerSize',5 ,'LineWidth', 1);
