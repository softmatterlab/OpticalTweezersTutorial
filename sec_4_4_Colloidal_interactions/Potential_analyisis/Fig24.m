...
load('data/propagator_traj_choices.mat','i_peak_peak','i_opt_peak','i_peak_opt','i_opt_opt');

for ik = 1:numel(ik_list)
    k = ik_list(ik);
    fh{k}=open(['data/propagator_' num2str(k) '.fig']);
    ah{k}=get(fh{k},'Children');
end


name_fig{1} = 'data/h3_xi10_NL.fig';
name_fig{2} = 'data/h10_xi24_NL.fig';
name_fig{3} = 'data/h11_xi28_NL.fig';
name_fig{4} = 'data/h13_xi34_NL.fig';
