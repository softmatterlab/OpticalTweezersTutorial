...
DATA=load('../data/data_feeding_cooling_vacuum.mat');
...
Ypix = by1+2*ywi+2*by2+bym;  % high in pixel
...
LL= legend ([p(1)  p(3) p(2) p(4) f(1)],'$\gamma_{\rm fb}$ =0.3 kHz','$\gamma_{\rm fb}$ =0.9 kHz',...
     '$\gamma_{\rm fb}$ =3.0 kHz','shotnoise','fit','Interpreter','latex')
 
LL.FontSize = 18;
LL.Box = 'off';
LL.NumColumns = 2;
LL.Box = 'off';
LL.Position= [0.1 0.61 0.25 0.1];
...
DATA=load('../data/data_temperature.mat')
DATAT=struct2table(DATA);
...
DATA=load('../data/data_heterodyne_spectra_vacuum.mat')
DATAT=struct2table(DATA);
...
for i=1: 3
 ...
 LL= legend ([p(1) p(2) p(3) p(4) ],'x','y', 'z','fit','Interpreter','latex','Box','off')
 
LL.FontSize = 18;
LL.Box = 'off';
LL.Position= [0.78 0.62 0.25 0.1]


ylabel('$\bar {\rm n}$','Interpreter', 'Latex')
