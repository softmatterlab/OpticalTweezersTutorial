...
%hold all;
rectangle('Position',[-60,0,600,70],'FaceColor',col3)
hold on
 ...
 plot([0 0],[-100 50],'--','LineWidth', 1.5, 'color','k')
   ...
   annotation('textarrow',[0.52,0.49],[0.7,0.7],'String','HV turned on','Interpreter','latex','FontSize',22)
    annotation('textarrow',[0.525,0.55],[0.87,0.845],'String','+1e','Interpreter','latex','FontSize',22)
        annotation('textarrow',[0.57,0.595],[0.89,0.87],'String','+2e','Interpreter','latex','FontSize',22)

 
   annotation('ellipse',[0.437 0.85 0.02 0.04 ],'LineWidth',1.5)
   text(-39.5, 36,'+','Interpreter','latex','FontSize',25)
   annotation('ellipse',[0.437 0.68 0.02 0.04 ],'LineWidth',1.5)
    text(-37.1, -43,'_','Interpreter','latex','FontSize',25)
...
p=semilogy(fm,PSDmean(:,1)/calibErik1^2,'o','color','k','LineWidth',1)
hold on
p1=semilogy((fitRange(1)+1:fitRange(2))'*(fm(2)-fm(1)),FittedCurve/calibErik1^2,'LineWidth',2,'color','r');
...
plot([4.53e4 4.53e4],[1e-20 1e-10],'--','LineWidth', 1.5, 'color','k')

%  text(4.3e4,2e-16,'f$_d$','Interpreter','latex','FontSize',22,'BackgroundColor', 'w')
 annotation('textarrow',[0.2,0.245],[0.7,0.65],'String','$f_{\rm d}$ ','Interpreter','latex','FontSize',22)
xlim([40.5 47.5]*1E3)
ylim([2E-19 3E-15]);
% set(p,'LineWidth',3);
% set(gca,'FontSize',fsize);
xlabel('$f$ (Hz)','Interpreter','latex','FontSize',30);
ylabel('PSD (m$^2$  Hz$^{-1})$','Interpreter','latex','FontSize',30);
