...
LL.Box = 'off';
LL.FontSize = 18;
LL.Position= [0.070 0.52 0.2164 0.3740]
LL.Interpreter='latex';

xlabel('Pressure $P_\mathrm{gas}$ (mbar)','Interpreter', 'latex')
ylabel('Heating Rate $\Gamma^\prime / (2\pi)$ (K/s)', 'Interpreter', 'latex')
...
%%
 load('data_heating_rates_power_1e-05.mat')
...
errorbar(power*1000,Gamma_power, Gamma_power_error, Gamma_power_error, power*100, power*100, '.',...
    'Color', [0.99, 0.56, 0.2 ],'LineWidth',1)

xlabel('Laser Power $P$ (mW)','Interpreter', 'latex')
ylabel('Heating Rate $\Gamma^\prime / (2\pi)$ (K\ s$^{-1}$)','Interpreter', 'latex')

% txt = '$P_{\rm gas}=1x10^5$ (mbar)','Interpreter', 'Latex')
text(0,7.5,'$P_{\rm gas}$=1x10$^{-5}$ (mbar)','Interpreter','latex','FontSize',22)
set(gca,'TickLabelInterpreter','latex', 'linewidth',1.5,'FontSize',25, 'XMinorTick','on','xlim',[-1, 15])


%%
load('data_frequency_shif.mat')
N=length(T);
...
hold on 
for i=1:N-1
p(1)=scatter(T(i), delta_freq(i),55,'o','MarkerEdgeColor',colorMap(i,:),'LineWidth', 1)
hold on
end


xlabel('$T_{\rm part}$ (K)','Interpreter', 'latex')
