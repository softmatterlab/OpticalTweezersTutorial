...
for i=1:length(F)
    if (SS==0) 
        ...
        fid1=fopen(char(strcat(file(1:end-4),'_',num2str(Ntraj),'T_U.txt')),'a');
    ...
    elseif (SS==1&&F(i)>MaxF)
        ...
        fid2=fopen(char(strcat(file(1:end-4),'_',num2str(Ntraj),'T_D.txt')),'a');
        fprintf(fid2,'%s\t%s\n','#Y dist(nm)', 'Y force(pN)');
        SS=2;
        fprintf(fid2,'%f\t%f\n',E(i),F(i));
...
for i=2:Ntraj-1
    %File where we will write Y distance and Y force of  UP trajectory
    fid4=fopen(char(strcat(file(1:end-4),'_',num2str(i),'T_U.txt')),'r');
    fid5=fopen(char(strcat(file(1:end-4),'_',num2str(i),'T_D.txt')),'r');
...
fid6=fopen(char(strcat(file(1:end-4),'_Stiffness_U.txt')),'a');
fid7=fopen(char(strcat(file(1:end-4),'_Stiffness_D.txt')),'a');
