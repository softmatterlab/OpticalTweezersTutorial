function [k_acf_lf,sigma_k_acf_lf,D_acf_lf,sigma_D_acf_lf,gamma_acf_lf, sigma2_gamma_acf_lf , k_acf_nl, sigma_k_acf_nl, D_acf_nl, sigma_D_acf_nl,gamma_acf_nl, sigma2_gamma_acf_nl, tau0_exp_lf, tau0_exp_nl,Xpix, Ypix]=plotsub_pot(filename, positioninthefig1, title1, T, subsample, partau0, end_plot)
load(filename);
disp(filename);
kb=1.38064852e-23;
...
%linear fit
[k_acf_lf, sigma_k_acf_lf, D_acf_lf, sigma_D_acf_lf,gamma_acf_lf, sigma2_gamma_acf_lf,tau_acf_lf, mc, Ec,indc, tau0_exp_lf, c0_exp_lf]=acf_lfit(x(1:subsample:size(x,1),:),T,dt*subsample);
%[k_acf_lf, Ek_acf, D_acf, ED_acf, tau_lf, mc, Ec,indc, tau0_exp_lf, c0_exp_lf]=acf_lfit(x(1:subsample:size(x,1),:),T,dt*subsample);
%non linear fit
[k_acf_nl, sigma_k_acf_nl, D_acf_nl, sigma_D_acf_nl,gamma_acf_nl, sigma2_gamma_acf_nl, tau_nl, mc, Ec, indc, tau0_exp_nl, c0_exp_nl]=acf_nlfit(x(1:subsample:size(x,1),:),T,dt*subsample);
...
xt = [bx1-70,bx1+xwi+bx1-70,2*bx1+2*xwi+bx1-50];
